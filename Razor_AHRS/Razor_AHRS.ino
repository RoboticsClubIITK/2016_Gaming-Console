...
//Flex
 ...
 int fingerstate2;             
 int fingbool2=1; 
 int lastfingerstate1 = 1;
 int lastfingerstate2 = 1;  
 long fingerlastDebounceTime1 = 0;
 long fingerlastDebounceTime2 = 0;    
 long fingerdebounceDelay = 100;
 int avg1=170;
 int avg2=170;
...
void setup()
{
  ...
  BTSerial.begin(OUTPUT__BAUD_RATE);
  // Init status LED
...
timep=millis();
 ...
 timeg = (millis()-timep)/1000.0;
    timep=millis(); 

    
    fingbool1= (analogRead(FingerV1))/avg1; 
    fingbool2= (analogRead(FingerV2))/avg2; 
    if (fingbool1 != lastfingerstate1) { 
      fingerlastDebounceTime1 = millis();  // reset the debouncing timer 
    }  
    if (fingbool2 != lastfingerstate2) { 
       fingerlastDebounceTime2 = millis();  // reset the debouncing timer 
    }   
    if ((millis() - fingerlastDebounceTime1) > fingerdebounceDelay) { 
    if (fingbool1 != fingerstate1) { 
       fingerstate1 = fingbool1; 
       if (fingbool1 == 0) { 
 ...
 lastfingerstate1 = fingbool1; 
 lastfingerstate2 = fingbool2;
 //Serial.println(timeg*1000); 
 delay(0);
